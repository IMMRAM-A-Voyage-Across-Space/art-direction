# IMMRAM — World Bible

**IMMRAM**

*World Bible · Art & Naming Reference*

> A Voyage Across Space — the grandchildren of the immrama sail a clan-host isle to mythic isle,
> permadeath at every landing.

| Isles | Causes | Units | Commanders | Wild things |
|---|---|---|---|---|
| 6 | 12 | 36 | 18 | 11 |

## The through-line

**Every war here is a family argument fought with the same hands.**

**IMMRAM** is an **immram** — a voyage-tale in the old Irish way. Its cast are the **grandchildren of
the immrama**: descendants of the legendary voyagers (Máel Dúin, Bran, Brendan) who inherit the old
sea-roads and sail them out among the stars. A single biome breeds **one ecology**: one shared
six-tier roster. **Two causes** lay claim to it under opposing banners, so the army you command is
the army you meet across the table — *the same crew, rowing for a different chief.* This document is
the art brief for that world: its look, its naming, and every drawing an artist needs — with a
checklist per isle.

Planet · Biome
:   Each isle has a mythic planet-name and a biome-name. Art is authored per biome; the fort/citadel
    is themed per world.

Naming
:   Irish first, English gloss after. Unit tiers run T1 (chaff) → T6 (champion) across three
    classes: melee, ranged, flyer.

Deliverables
:   Every isle has a checklist of exactly what to draw — terrain, structures, units, commanders, and
    its apex beast.

---

## The Voyage of the Grandchild

*The story we're sailing.*

A story **tutorial-campaign** sits atop the game; the game itself stays a **roguelike-lite** —
procedural voyages, permadeath. The campaign follows **Máel Dúin's grandchild**, who inherits an
unfinished wound and a fixed destination: the ones responsible. Across a handful of isles the
grandchild is changed, until — like the ancestor — they can **choose mercy instead of the kill**.

!!! note "The turn is the whole design"

    The campaign is a voyage with one fixed destination — vengeance. Completing it, by choosing
    forgiveness, is what **releases** the player into the open sea: the endless roguelike, with no
    destination at all. Letting go of the one place you were sailing toward is what opens the whole
    ocean.

!!! warning "The one rule — echoes, not transcription"

    We inherit the mood and shape of the old tales; we never re-stage them literally.

    **Do**

    - Inherit the **mood and shape** of an ancestral wonder — invent the matter in space
    - “Ants that eat the boat” → a scavenger-swarm stripping a derelict
    - “Land of eternal youth” → a planet that tempts you to stop voyaging
    - Cloud · Tree · Star recur as half-remembered legend-motifs, not stage props

    **Don't**

    - Re-stage the tales literally (no literal Isle of Ants)
    - Treat any island list as a locked 1:1 map
    - Lose the grandchildren's distance — they half-remember these legends
    - Forget the mood arc: grief/hardness early, wonder/release as it opens out

!!! tip "Two bloods per isle — a visual tell"

    One cause is led by **grandchildren of the immrama** (the rooted side that tends the isle) —
    draw them **carrying their lineage**: knotwork bloodline-marks, an ancestral crest worn like a
    badge. The rival cause are the **“new”** (the side that consumes or remakes it) — draw their
    heraldry **bare of the old marks, or effacing them**. Whether they were ever kin is a mystery
    the art should *hint at, never answer.*

### The bloodlines

*Which ancestor each rooted cause descends from. Provisional first pass. Each rooted cause is the
grandchildren of one immram voyager — read that ancestor's tale to know the house.*

| Isle · rooted cause | Ancestor |
|---|---|
| Mag Mell · Tuatha Blátha | **Máel Dúin's line** — the wanderer who chose mercy over the kill (your campaign home) |
| Tír na nÓg · Na Draoithe | **Bran's line** — voyager to the deathless Otherworld — Tír na nÓg itself |
| Gorias · Na Fomhóraigh | **the Uí Chorra's line** — raiders redeemed, who rebuild from the wreck |
| Murias · Dún Seaca | **Snedgus & Mac Riagla's line** — two clerics who endured a killing sea |
| Findias · Na Gaibhne | **Brendan's line** — past the isle of fire-smiths and the holy mountain |
| Tech Duinn · Cumann na Piocóide | **a lost line** — the founding voyage was never written down *(deliberately open — feeds the mystery)* |

### The source tales — the well

Read for flows, moods, naming, and icons; then bend to our palette and pixel medium.

- [CELT — full texts (the primary well)](https://celt.ucc.ie/transpage.html)
- [Máel Dúin (Stokes) — ~30 islands](https://celt.ucc.ie/transpage.html)
- [Uí Chorra (Stokes)](https://celt.ucc.ie/published/T303030.html)
- [Uí Corra — fast commentary](https://www.medievaldeathtrip.com/the-voyage-of-the-ui-corra-part-1/)
- [The Voyage of Bran](https://archive.org/details/voyageofbransono01meye)
- [St Brendan / Navigatio](https://archive.org/details/voyageofsaintbre0000unse)
- [Tolkien's “Imram” (Cloud·Tree·Star)](https://tolkiengateway.net/wiki/Imram)

---

## Neon Celtic Pop

*The look.*

Three things at once. Retro-futurist neon, Insular-Celtic ornament, and pop-poster punch — chunky
pixel art where a vast dark ground makes a few saturated neons sing, and gold Celtic knotwork lays
the *ancient* over the *futuristic*. Think a Book-of-Kells page rebuilt as a neon arcade cabinet.

### Three pillars, one look

<div markdown>
| | Pillar | What it means |
|---|---|---|
| <span class="swatch" style="background:#08060f"></span> | **Neon on the dark** | Space is vast and near-black. Grounds stay dark and desaturated so a small number of glowing neons carry all the energy. Bloom lives on the neons only — never the grounds. |
| <span class="swatch" style="background:#e8b83a"></span> | **Celtic gold** | Insular-Irish ornament in gold — interlace, spirals, key-frets — trims chrome, structures, and emblems. It reads as sacred craft laid over the tech, and blooms softly like the other lights. |
| | **Pop punch** | Bold, high-contrast, readable. Chunky silhouettes you recognise at a glance; clean shapes; nothing muddy or fussy. If it doesn't read as a poster at thumbnail size, simplify. |
</div>

### What it looks like

- Dark ground · 1–2 neon accents · bloom on the light
- Gold Celtic ornament over a tech panel
- Bold silhouette · reads at thumbnail size

### Colour — guidance, not a lock

Each world has its own mood (the green plain, the rust dunes, the ice, the lava, the bone-grey
moon). Pick what serves the scene — these are the only hard rules:

<div markdown>
- Grounds near-black & desaturated — <span class="swatch" style="background:#08060f"></span> `#08060f`
- 1–2 saturated neons per scene
- Gold = all ornament — <span class="swatch" style="background:#e8b83a"></span> `#e8b83a`
- Bloom on neon + gold only
- Player = cyan rim <span class="swatch" style="background:#4ff2ff"></span> `#4ff2ff` · Enemy = magenta rim <span class="swatch" style="background:#ff4fd8"></span> `#ff4fd8`
</div>

The exact ground, ornament-gold and rim values are locked on the [Palette](palette.md) page; the
per-isle accents below are the mood, not the whole set.

!!! tip "Lean in"

    - Silhouette first, detail second
    - Consistent top-left key light across everything
    - Let big dark areas breathe
    - Save saturation for what matters

!!! danger "Avoid"

    - Rainbow scenes — too many competing neons
    - Bloom/haze on terrain and mid-tones
    - Filigree so fine it won't survive the pixel grid
    - Muddy mid-value soup with no black anchor

### Reference wells — the other two thirds

The source tales feed the *myth*; these feed the *look*. Read for calibration, then bend to our
palette and pixel grid.

**Space — vast & empty**

- [1970s SF paperback art](https://www.counter-print.co.uk/products/worlds-beyond-time-sci-fi-art-of-the-1970s)
  — Chris Foss, **John Harris** (monumental structures in hazy light), Peter Elson
- [Moebius / *Arzach*](https://en.wikipedia.org/wiki/Jean_Giraud) — the lone-voyage-through-wonder
  mood (clean horizons; not his gritty cyberpunk)

**Neon pop — retro-futurist skin**

- [Signalnoise “Outrun”](https://signalnoise.com/outrun-series) — magenta/cyan/violet neon; our
  cyan/magenta rim logic
- [Hyper Light Drifter](https://en.wikipedia.org/wiki/Hyper_Light_Drifter) — the north star: pixel
  art, neon-on-dark. If you look at one, this

No reference has our actual fusion — illuminated-manuscript **gold knotwork over synthwave neon**.
These wells feed the halves; the cross is ours.

---

## Celtic Filigree

*The ornament layer.*

The gold vocabulary — the curls, twists, and knots that mark this world as Celtic. Use it on UI
chrome, structure trim, faction emblems, and borders; keep it off unit silhouettes so they still
read. These are motif references, not final pixel art — the drawing job is to translate them into
the chunky gold-on-dark look above.

| Motif | Reads as |
|---|---|
| Spiral (curl) | the single boss |
| Triskele | triple spiral |
| Double spiral | S-scroll |
| Key fret | step / meander border |
| Plait (twist) | two-strand weave |
| Knot | interlace ring |

---

## 01 · Mag Mell

*Machaire · green plain · water*

<div markdown>
**Accent:** <span class="swatch" style="background:#5f82ff"></span> `#5f82ff`
</div>

The clan-host's first landfall — the Plain of Delight, broken and sown into the **breadbasket of the
star-sea**. “Mag Mell” became the crews' word for *the good years*.

IMMRAM's first war is a **civil** one: settler-line against admiralty, the same folk split over who
the plain belongs to. Familiar, warm, and quietly heartbreaking — which is why you start here.

### Two causes · opposed banners

#### Tuatha Blátha — folk of blossom

<div markdown>
**Banner:** <span class="swatch" style="background:#1fbf5f"></span> `#1fbf5f`
</div>

The settler-line who broke the plain by hand; hold ground others would abandon. Economy & defense.

#### An Ard-Riail — the High Command · “The Directorate”

<div markdown>
**Banner:** <span class="swatch" style="background:#ff3ec9"></span> `#ff3ec9`
</div>

The centralised admiralty and its plasma-armed hosting. Logistics & firepower.

### Commanders · 3 per isle

#### Bríd, the Kindler *(Tuatha Blátha)*

Quote
:   *“We didn't break this plain to hand it back. Dig in.”*

Role
:   Entrenches and outlasts; spends ammunition like water in a drought.

#### Medb, the Ledger-Queen *(Ard-Riail)*

Quote
:   *“Sentiment is a supply problem. I have solved it.”*

Role
:   Wins by out-growing the enemy — fresh stacks faster than they fall.

#### Fergus, the Ironside *(hardliner)*

Quote
:   *“It held because I was on it.”*

Role
:   Stands in the front rank taking the retaliation; rallies the line.

### Shared roster · both causes field these six

*Machaire ecology.*

| Tier | Class | Unit | Gloss | Role & signature |
|---|---|---|---|---|
| T1 | MEL | Óglach | volunteer-warrior | Swarm chaff; soaks retaliation |
| T2 | RNG | Ceithearnach | kern | Line infantry; overwatch specialist |
| T3 | FLY | Fiach | raven scout | High sight; reveals fog, fast flank |
| T4 | MEL | Gallóglach | gallowglass | Heavy tank; strong retaliation, tramples cover |
| T5 | FLY | Badhbh | war-crow | Artillery flyer; long-range strike |
| T6 | MEL | Fomhórach | giant | Champion; ignores cover, terror |

### Structures

Capital · boss
:   **Space Castle** — themed fort, the planet's stronghold, and the **only built structure unique to
    this isle** (Colossus canvas).

Recruitment
:   **Recruitment Hall**

Crest · Tuatha Blátha
:   Faction emblem — heraldic mark in this cause's banner colour.

Crest · An Ard-Riail
:   Faction emblem — heraldic mark in this cause's banner colour.

Shared — see Universal
:   Dropship pad · Mines · Mercenaries — drawn once for the whole game, not per planet.

### Apex wildlife · the monster the isle itself made

**Glas Gaibhleann** — *the Grazer-Titan*

The mythic cow of endless plenty, grown monstrous. “The plain was theirs first.” A single charge
tramples an entire rank flat.

Belongs to this isle — draw the **beast** and its **recruitment hall** (its den).

### Deliverables — Mag Mell

??? note "Deliverables — 24 items"

    **Terrain & features**

    - [ ] 3× Mountains
    - [ ] 3× Boulders
    - [ ] 3× Forest
    - [ ] 2× Flower patch
    - [ ] 1× Wheat field
    - [ ] 1× River
    - [ ] 1× Bridge over river
    - [ ] 1× Ferry dock
    - [ ] 1× Beach
    - [ ] 1× Path

    **Structure — capital only**

    - [ ] Space Castle

    **Faction crests**

    - [ ] Tuatha Blátha — crest
    - [ ] An Ard-Riail — crest

    **Units (T1–T6)**

    - [ ] T1 Óglach — volunteer-warrior
    - [ ] T2 Ceithearnach — kern
    - [ ] T3 Fiach — raven scout
    - [ ] T4 Gallóglach — gallowglass
    - [ ] T5 Badhbh — war-crow
    - [ ] T6 Fomhórach — giant

    **Commanders**

    - [ ] Bríd, the Kindler
    - [ ] Medb, the Ledger-Queen
    - [ ] Fergus, the Ironside

    **Apex wildlife (beast + its hall)**

    - [ ] Glas Gaibhleann — the Grazer-Titan
    - [ ] Glas Gaibhleann — recruitment hall

---

## 02 · Tír na nÓg

*Draíocht · bioluminescent growth · algal water*

<div markdown>
**Accent:** <span class="swatch" style="background:#ffe24a"></span> `#ffe24a`
</div>

Not so much an isle as a single breathing thing: one luminous mycelial web that **remembers**,
thinking slowly in spore-song and chemical tides.

Every beast you field is grown from the same biomass. The only question is whether that life is a
**song to be kept in tune**, or clay to be remade.

### Two causes · opposed banners

#### Na Draoithe — the Druids

<div markdown>
**Banner:** <span class="swatch" style="background:#1fd6a8"></span> `#1fd6a8`
</div>

The isle waking to defend itself: a gestalt hive-mind, harmony and regeneration. Sustain & biomass.

#### Na Beangáin — the grafts · “The Grafted”

<div markdown>
**Banner:** <span class="swatch" style="background:#ff6f8f"></span> `#ff6f8f`
</div>

Healers who stopped studying the isle and started *wearing* it. Mutation & aggression.

### Commanders · 3 per isle

#### Dian Cécht *(Na Draoithe)*

Quote
:   *“Nothing is lost here. It is only re-sung.”*

Role
:   Sings to keep the chorus whole; heals as a forest heals a burn.

#### Miach, the Grafter *(Beangáin)*

Quote
:   *“Why inherit a body when you can author one?”*

Role
:   Grows stronger from everything that dies near him.

#### Naomh Folamh, the Hollow Saint *(zealot)*

Quote
:   *“Be still. The bloom is coming for you, and it is patient.”*

Role
:   Leads a tide of sporelings; nearby enemies forget their orders.

### Shared roster · both causes field these six

*Draíocht ecology.*

| Tier | Class | Unit | Gloss | Role & signature |
|---|---|---|---|---|
| T1 | MEL | Síolán | seedling | Swarm chaff; bursts into a spore cloud on death |
| T2 | RNG | Féithleog | tendril | Tendril whip; snares, zone control |
| T3 | FLY | Leamhan Sí | fairy-moth | Reveals fog and blinds |
| T4 | MEL | Sligeach | shell-beast | Tank; chitin armor, regenerates, slow |
| T5 | RNG | Máthair Shíl | seed-mother | Artillery; spore-bombs, spawns free Síolán |
| T6 | MEL | An Bile | the World-Tree | Champion; regenerates each round, terror |

### Structures

Capital · boss
:   **Newgrange-style Fort** — themed fort, the planet's stronghold, and the **only built structure
    unique to this isle** (Colossus canvas).

Recruitment
:   **Mushroom-Hall**

Crest · Na Draoithe
:   Faction emblem — heraldic mark in this cause's banner colour.

Crest · Na Beangáin
:   Faction emblem — heraldic mark in this cause's banner colour.

Shared — see Universal
:   Dropship pad · Mines · Mercenaries — drawn once for the whole game, not per planet.

### Apex wildlife · the monster the isle itself made

**Gaoth Sí** — *the Pollen Wraith*

The fairy-wind that steals wits — a drifting veil of spores with no body to kill. Even the living
isle fears it.

Belongs to this isle — draw the **beast** and its **recruitment hall** (its den).

### Deliverables — Tír na nÓg

??? note "Deliverables — 24 items"

    **Terrain & features**

    - [ ] 3× Mountains
    - [ ] 3× Giant Mushroom Forest
    - [ ] 3× Mushroom Forest
    - [ ] 2× Small mushroom patch
    - [ ] 1× Sparkly mist
    - [ ] 1× River
    - [ ] 1× Bridge over river
    - [ ] 1× Ferry dock
    - [ ] 1× Beach
    - [ ] 1× Path

    **Structure — capital only**

    - [ ] Newgrange-style Fort

    **Faction crests**

    - [ ] Na Draoithe — crest
    - [ ] Na Beangáin — crest

    **Units (T1–T6)**

    - [ ] T1 Síolán — seedling
    - [ ] T2 Féithleog — tendril
    - [ ] T3 Leamhan Sí — fairy-moth
    - [ ] T4 Sligeach — shell-beast
    - [ ] T5 Máthair Shíl — seed-mother
    - [ ] T6 An Bile — the World-Tree

    **Commanders**

    - [ ] Dian Cécht
    - [ ] Miach, the Grafter
    - [ ] Naomh Folamh, the Hollow Saint

    **Apex wildlife (beast + its hall)**

    - [ ] Gaoth Sí — the Pollen Wraith
    - [ ] Gaoth Sí — recruitment hall

---

## 03 · Gorias

*Meirg · rust-red iron dunes · acid*

<div markdown>
**Accent:** <span class="swatch" style="background:#4ee06a"></span> `#4ee06a`
</div>

A graveyard that learned to walk. An earlier, prouder wave choked on the acid air and died in
thousands, leaving a rust desert under the bones of drowned hulls.

Both hosts ride the same salvage-walkers. One patches them like heirlooms; the other lets them rot
into something crueler **on purpose**. Build from the ruin — or finish what the rust started?

### Two causes · opposed banners

#### Na Fomhóraigh — Fomorian raiders

<div markdown>
**Banner:** <span class="swatch" style="background:#ff3b30"></span> `#ff3b30`
</div>

Scrapper clans: a wreck is a promise, a salvaged bolt is sacred. Salvage & attrition.

#### An Creimeadh — the corrosion

<div markdown>
**Banner:** <span class="swatch" style="background:#33e6f0"></span> `#33e6f0`
</div>

An acid-doctrine war-state: nothing deserves to last, the rust is holy. Corrosion & terror.

### Commanders · 3 per isle

#### Balor of the Evil Eye *(Na Fomhóraigh)*

Quote
:   *“Everything that dies on my sand pays rent.”*

Role
:   War chest swells off the dead; welds his mechs back mid-fight.

#### Bres, the Blighted *(Creimeadh)*

Quote
:   *“All things return to rust. I merely keep the schedule.”*

Role
:   Eats the enemy's defenses away layer by layer, then it rains.

#### Fáidh na Meirge, the Rust Prophet *(warlord)*

Quote
:   *“The desert doesn't count its dead. Neither do I. Surge.”*

Role
:   Conjures fresh waves out of the wreckage faster than they fall.

### Shared roster · both causes field these six

*Meirg ecology.*

| Tier | Class | Unit | Gloss | Role & signature |
|---|---|---|---|---|
| T1 | MEL | Meirgín | little-rust | Swarm chaff; explodes on death |
| T2 | RNG | Aigéadóir | acid-caster | Corrosive sprayer; applies Defense-shred |
| T3 | FLY | Sciathán Meirge | rust-wing | Fast, fragile salvage-strafe; ignores dunes |
| T4 | MEL | Carnán | scrap-heap | Tank; absorbs wreckage to self-repair, slow |
| T5 | RNG | Creimire | the corroder | Artillery; acid shells leave a lingering pool |
| T6 | MEL | Iarnphéist | iron-worm | Champion; melts cover and armor, terror |

### Structures

Capital · boss
:   **Mad-Max Fort** — themed fort, the planet's stronghold, and the **only built structure unique to
    this isle** (Colossus canvas).

Recruitment
:   **Scrap-Yard**

Crest · Na Fomhóraigh
:   Faction emblem — heraldic mark in this cause's banner colour.

Crest · An Creimeadh
:   Faction emblem — heraldic mark in this cause's banner colour.

Shared — see Universal
:   Dropship pad · Mines · Mercenaries — drawn once for the whole game, not per planet.

### Apex wildlife · the monster the isle itself made

**Oilliphéist** — *the Dune Serpent*

The great Irish worm that carved rivers, swimming the rust like water. It surfaces *under* whatever's
heaviest. The Corrosion calls it a saint.

Belongs to this isle — draw the **beast** and its **recruitment hall** (its den).

### Deliverables — Gorias

??? note "Deliverables — 24 items"

    **Terrain & features**

    - [ ] 3× Mountains
    - [ ] 3× Rusting hulks (wrecks)
    - [ ] 3× Dead-tree forests
    - [ ] 2× Small rusting hulks
    - [ ] 1× decorative feature (TBD — to figure out)
    - [ ] 1× Acid River
    - [ ] 1× Bridge over river
    - [ ] 1× Zipline across acid
    - [ ] 1× Beach
    - [ ] 1× Path

    **Structure — capital only**

    - [ ] Mad-Max Fort

    **Faction crests**

    - [ ] Na Fomhóraigh — crest
    - [ ] An Creimeadh — crest

    **Units (T1–T6)**

    - [ ] T1 Meirgín — little-rust
    - [ ] T2 Aigéadóir — acid-caster
    - [ ] T3 Sciathán Meirge — rust-wing
    - [ ] T4 Carnán — scrap-heap
    - [ ] T5 Creimire — the corroder
    - [ ] T6 Iarnphéist — iron-worm

    **Commanders**

    - [ ] Balor of the Evil Eye
    - [ ] Bres, the Blighted
    - [ ] Fáidh na Meirge, the Rust Prophet

    **Apex wildlife (beast + its hall)**

    - [ ] Oilliphéist — the Dune Serpent
    - [ ] Oilliphéist — recruitment hall

---

## 04 · Murias

*Oighear · methane ice · liquid methane*

<div markdown>
**Accent:** <span class="swatch" style="background:#ff6ad8"></span> `#ff6ad8`
</div>

Murias should have killed its landing in the first winter. The cold here is not weather; it is the
law, the enemy, and the landlord.

The colossus that shields one host's line is the same beast that freezes the other's prey. Choose
between **the wall and the ambush**.

### Two causes · opposed banners

#### Dún Seaca — the frost-fort · “Frostbound Hold”

<div markdown>
**Banner:** <span class="swatch" style="background:#2e8bff"></span> `#2e8bff`
</div>

Survival is endurance: hold the wall, yield nothing. Fortification & defense.

#### An Díorma Oighir — the ice-raiders · “Glacial Vanguard”

<div markdown>
**Banner:** <span class="swatch" style="background:#ff8a1e"></span> `#ff8a1e`
</div>

The cold is a blade — freeze an enemy mid-step, be gone before the storm clears. Ambush & control.

### Commanders · 3 per isle

#### Cathair, the Warden *(Dún Seaca)*

Quote
:   *“We do not take ground. We refuse to give it.”*

Role
:   Raises ice ramparts; a stack that plants its feet is nearly immovable.

#### Scáthach, the Vanguard *(Díorma)*

Quote
:   *“The cold is already inside your line. I just told it where to bite.”*

Role
:   Steals an enemy's turn out from under them; strikes and vanishes.

#### An Doleáite, the Thawless *(zealot)*

Quote
:   *“Cold is a question. We stopped answering.”*

Role
:   Ignores enemy freeze; seals a whole field of ice against all but her own.

### Shared roster · both causes field these six

*Oighear ecology.*

| Tier | Class | Unit | Gloss | Role & signature |
|---|---|---|---|---|
| T1 | MEL | Oighrín | little-ice | Swarm chaff; chill-on-hit |
| T2 | RNG | Siocaire | froster | Freeze-rifle; applies Slow |
| T3 | FLY | Biorán Reo | frost-dart | Ice-shard strafe, reveals fog, brittle |
| T4 | MEL | Famhair Oighir | ice-giant | Tank; immune to slow, very slow |
| T5 | RNG | Síobaire | blizzard-caster | Artillery; AoE freeze storm |
| T6 | MEL | An Reo Mór | the Freezer | Champion; freezes its attackers, terror |

### Structures

Capital · boss
:   **Frozen Spire Fortress** — themed fort, the planet's stronghold, and the **only built structure
    unique to this isle** (Colossus canvas).

Recruitment
:   **Ice-Hall**

Crest · Dún Seaca
:   Faction emblem — heraldic mark in this cause's banner colour.

Crest · An Díorma Oighir
:   Faction emblem — heraldic mark in this cause's banner colour.

Shared — see Universal
:   Dropship pad · Mines · Mercenaries — drawn once for the whole game, not per planet.

### Apex wildlife · the monster the isle itself made

**Péist Seaca** — *the Hoarfrost Drake*

The first winter, given wings. A breath that freezes a whole line; a shadow that leaves statues
behind.

Belongs to this isle — draw the **beast** and its **recruitment hall** (its den).

### Deliverables — Murias

??? note "Deliverables — 24 items"

    **Terrain & features**

    - [ ] 3× Mountains
    - [ ] 3× Ice Boulders
    - [ ] 3× Frozen forests
    - [ ] 2× Snow dunes
    - [ ] 1× Snowman
    - [ ] 1× Frozen River
    - [ ] 1× Path/bridge over river
    - [ ] 1× Sled dogs (ice crossing)
    - [ ] 1× Beach
    - [ ] 1× Path

    **Structure — capital only**

    - [ ] Frozen Spire Fortress

    **Faction crests**

    - [ ] Dún Seaca — crest
    - [ ] An Díorma Oighir — crest

    **Units (T1–T6)**

    - [ ] T1 Oighrín — little-ice
    - [ ] T2 Siocaire — froster
    - [ ] T3 Biorán Reo — frost-dart
    - [ ] T4 Famhair Oighir — ice-giant
    - [ ] T5 Síobaire — blizzard-caster
    - [ ] T6 An Reo Mór — the Freezer

    **Commanders**

    - [ ] Cathair, the Warden
    - [ ] Scáthach, the Vanguard
    - [ ] An Doleáite, the Thawless

    **Apex wildlife (beast + its hall)**

    - [ ] Péist Seaca — the Hoarfrost Drake
    - [ ] Péist Seaca — recruitment hall

---

## 05 · Findias

*Gríosach · black volcanic rock · lava*

<div markdown>
**Accent:** <span class="swatch" style="background:#2ad6ff"></span> `#2ad6ff`
</div>

The forge at the end of the voyage — the sword-city's namesake, where the fire is the only thing
that ever loved anyone. All of it comes up out of the lava, so the lava became god.

The same magma-drakes march under both banners. Being the final isle, it asks the oldest question
its fire ever posed: does the flame exist to **forge, or to consume?**

### Two causes · opposed banners

#### Na Gaibhne — the smiths · “Emberwrights”

<div markdown>
**Banner:** <span class="swatch" style="background:#b6f53a"></span> `#b6f53a`
</div>

Forge-priests who venerate fire as a *maker*. Armor & industry.

#### Clann na Luaithe — the ash-born · “Cinderborn”

<div markdown>
**Banner:** <span class="swatch" style="background:#a94ce6"></span> `#a94ce6`
</div>

Heretics who venerate fire as a *destroyer* — burn brightest closest to death. Sacrifice & frenzy.

### Commanders · 3 per isle

#### Goibniu, the Forgemaster *(Na Gaibhne)*

Quote
:   *“Fire does not destroy iron. It teaches it shape.”*

Role
:   Reforges what shatters mid-battle; armors what survives.

#### Aodh, the Ashlord *(Luaithe)*

Quote
:   *“I am the spark that doesn't care what it lands on.”*

Role
:   Leaves the enemy alight and dwindling; fights hottest as it burns out of control.

#### Naomh na Luaithe, the Cinder Saint *(fire-cult)*

Quote
:   *“Give the fire what it asks. It only ever asks for everything.”*

Role
:   Spends half a stack to make the other half burn twice as bright.

### Shared roster · both causes field these six

*Gríosach ecology.*

| Tier | Class | Unit | Gloss | Role & signature |
|---|---|---|---|---|
| T1 | MEL | Gríosán | little-ember | Swarm chaff; leaves burning ground on death |
| T2 | RNG | Lasaire | flamer | Flamethrower cone; Burn DoT |
| T3 | FLY | Séideán Tine | fire-gust | Fast firebomb strafe, ignores terrain |
| T4 | MEL | An Inneoin | the Anvil | Tank; molten armor, burns attackers, slow |
| T5 | RNG | Teilgeoir Gríosaí | ember-hurler | Artillery; lava-mortar leaves a pool |
| T6 | MEL | Coire Lasrach | the Flaming Cauldron | Champion; living volcano, AoE burn aura, terror |

### Structures

Capital · boss
:   **Lava Fortress** — themed fort, the planet's stronghold, and the **only built structure unique to
    this isle** (Colossus canvas).

Recruitment
:   **Forge-Hall**

Crest · Na Gaibhne
:   Faction emblem — heraldic mark in this cause's banner colour.

Crest · Clann na Luaithe
:   Faction emblem — heraldic mark in this cause's banner colour.

Shared — see Universal
:   Dropship pad · Mines · Mercenaries — drawn once for the whole game, not per planet.

### Apex wildlife · the monster the isle itself made

**Aillén** — *the Magma Leviathan*

Aillén the Burner, who torched Tara each Samhain — a slow mountain of living magma that doesn't
attack so much as *arrive*.

Belongs to this isle — draw the **beast** and its **recruitment hall** (its den).

### Deliverables — Findias

??? note "Deliverables — 24 items"

    **Terrain & features**

    - [ ] 3× Mountains / Volcanoes
    - [ ] 3× Lava flows
    - [ ] 3× Needle Rock Formations
    - [ ] 2× Bonfires
    - [ ] 1× Little lava crack
    - [ ] 1× Lava river
    - [ ] 1× Bridge over river
    - [ ] 1× Zipline over lava
    - [ ] 1× Lava beach
    - [ ] 1× Path

    **Structure — capital only**

    - [ ] Lava Fortress

    **Faction crests**

    - [ ] Na Gaibhne — crest
    - [ ] Clann na Luaithe — crest

    **Units (T1–T6)**

    - [ ] T1 Gríosán — little-ember
    - [ ] T2 Lasaire — flamer
    - [ ] T3 Séideán Tine — fire-gust
    - [ ] T4 An Inneoin — the Anvil
    - [ ] T5 Teilgeoir Gríosaí — ember-hurler
    - [ ] T6 Coire Lasrach — the Flaming Cauldron

    **Commanders**

    - [ ] Goibniu, the Forgemaster
    - [ ] Aodh, the Ashlord
    - [ ] Naomh na Luaithe, the Cinder Saint

    **Apex wildlife (beast + its hall)**

    - [ ] Aillén — the Magma Leviathan
    - [ ] Aillén — recruitment hall

---

## 06 · Tech Duinn

*Cnámh · bone-grey dust & craters · the Veil*

<div markdown>
**Accent:** <span class="swatch" style="background:#ff5a5a"></span> `#ff5a5a`
</div>

Bone-grey, airless, and rich — a low-gravity moon-isle threaded with mines. But the deep
crater-floors hold **the Veil**: drifting banks of cold-trapped grey mist, disorienting and quietly
lethal.

The loud many against the silent few, over the same tunnels — and the fog between them. Learn which
paths through the mist are real, and who will lead you down them.

### Two causes · opposed banners

#### Cumann na Piocóide — the miners' union · “Pickaxe Union”

<div markdown>
**Banner:** <span class="swatch" style="background:#ffcf3a"></span> `#ffcf3a`
</div>

A labor-militia of hardhats and drilling drones; win in numbers. Economy & swarm.

#### An tOrd Tostach — the silent order

<div markdown>
**Banner:** <span class="swatch" style="background:#6a4cff"></span> `#6a4cff`
</div>

A vacuum-monastic sect sworn to quiet and dark — the only ones who know the Veil. Stealth &
precision.

### Commanders · 3 per isle

#### Sreng, the Foreman *(Cumann)*

Quote
:   *“One drill breaks. Ten thousand don't stop.”*

Role
:   Floods the dust with bodies and machines; refuses, at scale, to lose.

#### Tost, the Silent *(An tOrd)*

Quote
:   *“[no transmission]”*

Role
:   Opens every battle gone from the enemy's sensors; marks a target for the Order's fire.

#### Díthreabhach an Fholúis, the Vacuum Hermit *(savant)*

Quote
:   *“Gravity is a suggestion. Distance is a habit. I have quit both.”*

Role
:   Flyers and drones move farther and see more than physics should allow.

### Shared roster · both causes field these six

*Cnámh ecology.*

| Tier | Class | Unit | Gloss | Role & signature |
|---|---|---|---|---|
| T1 | MEL | Pollaire | the digger | Swarm chaff; self-destruct on death |
| T2 | RNG | Diúracóir | mass-driver | Railgun; long range, ignores cover |
| T3 | FLY | Súileach | the many-eyed | Best scout; widest sight, very fragile |
| T4 | MEL | Tarathar | the auger | Tank; drill-exosuit, breaches cover, slow |
| T5 | FLY | Ga Fada | the far-ray | Artillery; precise long-range kinetic strike |
| T6 | RNG | Lia Fáil | Stone of Destiny | Champion; planet-cracker rail pierces a whole line, terror |

### Structures

Capital · boss
:   **Cathedral** — themed fort, the planet's stronghold, and the **only built structure unique to
    this isle** (Colossus canvas).

Recruitment
:   **Ossuary-Hall**

Crest · Cumann na Piocóide
:   Faction emblem — heraldic mark in this cause's banner colour.

Crest · An tOrd Tostach
:   Faction emblem — heraldic mark in this cause's banner colour.

Shared — see Universal
:   Dropship pad · Mines · Mercenaries — drawn once for the whole game, not per planet.

### Apex wildlife · the monster the isle itself made

**Cú Sí** — *the Void Stalker*

The spectral death-hound of the mist — unseen until the instant it isn't. On Tech Duinn, the dark has
teeth.

Belongs to this isle — draw the **beast** and its **recruitment hall** (its den).

### Deliverables — Tech Duinn

??? note "Deliverables — 24 items"

    **Terrain & features**

    - [ ] 3× Lunar Mountains
    - [ ] 3× Craters
    - [ ] 3× Large skeletal remains
    - [ ] 2× Bone monoliths
    - [ ] 1× Mine ruin
    - [ ] 1× Canyon (the gap)
    - [ ] 1× Bridge over canyon
    - [ ] 1× Fog-guide (waystones)
    - [ ] 1× Thick fog bank
    - [ ] 1× Path

    **Structure — capital only**

    - [ ] Cathedral

    **Faction crests**

    - [ ] Cumann na Piocóide — crest
    - [ ] An tOrd Tostach — crest

    **Units (T1–T6)**

    - [ ] T1 Pollaire — the digger
    - [ ] T2 Diúracóir — mass-driver
    - [ ] T3 Súileach — the many-eyed
    - [ ] T4 Tarathar — the auger
    - [ ] T5 Ga Fada — the far-ray
    - [ ] T6 Lia Fáil — Stone of Destiny

    **Commanders**

    - [ ] Sreng, the Foreman
    - [ ] Tost, the Silent
    - [ ] Díthreabhach an Fholúis, the Vacuum Hermit

    **Apex wildlife (beast + its hall)**

    - [ ] Cú Sí — the Void Stalker
    - [ ] Cú Sí — recruitment hall

---

## Wandering Mercenaries

*Cause-blind · recruited from wild dens on any isle.*

### Muir-fhoghlaithe — the Drifter Corsairs

**Class:** RNG

Outlaw gunmen who sell to whoever's winning. Every kill lines their purse — and yours.

### An Sluagh — the Rogue Drone Swarm

**Class:** FLY

The restless flying host — war-drones that forgot their orders. Hijacks an enemy mechanical stack.

### Na Díbergaigh — the Masterless Blades

**Class:** MEL

Masterless marauder-warriors: strike clean, strike first, without provoking retaliation.

### Fathach — the Derelict War-Mech

**Class:** MEL

A colossus from a crater — armor running on a war that ended before anyone alive was born.

### Lucht na Marbhnaí — the Void Psalmists

**Class:** RNG

The dirge-singers, who sing to the vacuum. Their hymns harden an ally's resolve or unstring an
enemy's.

---

## Universal Deliverables

*Shared across every isle.*

Player tech, resources, and the wandering mercenaries — drawn **once for the whole game**, never per
planet. *(Apex beasts and their halls live with their home isle, above.)*

**Mines — unique for the whole game (5 resources × deposit + extractor + refined icon)**

- [ ] Ór / Gold — deposit
- [ ] Ór — extractor
- [ ] Ór — refined icon
- [ ] Cruach / Iron — deposit
- [ ] Cruach — extractor
- [ ] Cruach — refined icon
- [ ] Ola / Oil — pool
- [ ] Ola — extractor
- [ ] Ola — refined icon
- [ ] Críostal / Crystal — formation
- [ ] Críostal — extractor
- [ ] Críostal — refined icon
- [ ] Caor Thine / Plasma — vent
- [ ] Caor Thine — extractor
- [ ] Caor Thine — refined icon

**Player tech — one each, whole game**

- [ ] Dropship pad (the single landing pad for the whole game)
- [ ] Currach Catha (balanced hull)
- [ ] Currach Mianaigh (mining hull)
- [ ] Scuab Tuinne (science hull)

**Mercenaries — the 5 wild bands (creature art)**

- [ ] Muir-fhoghlaithe, the Drifter Corsairs
- [ ] An Sluagh, the Rogue Drone Swarm
- [ ] Na Díbergaigh, the Masterless Blades
- [ ] Fathach, the Derelict War-Mech
- [ ] Lucht na Marbhnaí, the Void Psalmists

**Mercenary recruitment halls (5 · one per band)**

- [ ] Muir-fhoghlaithe — recruitment hall
- [ ] An Sluagh — recruitment hall
- [ ] Na Díbergaigh — recruitment hall
- [ ] Fathach — recruitment hall
- [ ] Lucht na Marbhnaí — recruitment hall

---

## Naming Glossary

*Appendix. The recurring Gaelic terms, with pronunciation-friendly glosses. Spellings want a
native-speaker pass before final.*

`Immram`
:   the voyage-tale — a clan-host sailing isle to isle

`Máel Dúin`
:   the campaign ancestor — his grandchild's voyage frames the story

`Ór`
:   gold → Credits (universal resource)

`Cruach`
:   steel → Alloy (common)

`Ola`
:   oil → the common refinery resource

`Caor Thine`
:   flame-bolt → Plasma (rare; gates T5)

`Críostal`
:   crystal → Crystal (rare; gates T6)

`Sleá Lugha`
:   Spear of Lugh — the Orbital Strike active

`Tobar Sláine`
:   the Healing Well — Dian Cécht's Bloom active (his own well, returned to him)

`Athchóiriú`
:   refitting — the Repair active (Balor, Goibniu)

`Slógadh`
:   the Hosting — the Rally active

`Currach`
:   the hide-boat — your dropship hulls

`Óglach / Gallóglach`
:   volunteer-warrior / gallowglass — Machaire's melee line

`MEL · RNG · FLY`
:   melee · ranged · flyer — the three unit classes

---

*IMMRAM — A Voyage Across Space (formerly STARFALL / SpaceHeroes) · art brief generated from
docs/lore + faction design*
